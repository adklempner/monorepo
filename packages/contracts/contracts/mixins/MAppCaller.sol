...
contract MAppCaller {
  ...
  function resolve(
    ...
    returns (bytes memory)
  {
    bytes4 resolveSelector = CounterfactualApp(appDefinitionAddress).resolveSelector();
    bytes memory payload = abi.encodeWithSelector(resolveSelector, appState);
    (bool success, bytes memory returnData) = appDefinitionAddress.staticcall(payload);
    require(success);
